#!/usr/bin/env python
# coding: utf-8

import os
import pandas as pd
from datetime import datetime
from dotenv import load_dotenv

# Import get_input_path and get_output_path functions from batch.py
import sys
import os
sys.path.append(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
from batch import get_input_path, get_output_path

load_dotenv()


def dt(hour, minute, second=0):
    return datetime(2023, 1, 1, hour, minute, second)


def test_integration():
    # Create the same test dataframe as used in Q3 (unit test)
    data = [
        (None, None, dt(1, 1), dt(1, 10)),
        (1, 1, dt(1, 2), dt(1, 10)),
        (1, None, dt(1, 2, 0), dt(1, 2, 59)),
        (3, 4, dt(1, 2, 0), dt(2, 2, 1)),
    ]

    columns = [
        "PULocationID",
        "DOLocationID", 
        "tpep_pickup_datetime",
        "tpep_dropoff_datetime",
    ]
    
    df_input = pd.DataFrame(data, columns=columns)
    
    # S3 configuration for LocalStack
    S3_ENDPOINT_URL = os.getenv('S3_ENDPOINT_URL', 'http://localhost:4566')
    
    options = {
        'client_kwargs': {
            'endpoint_url': S3_ENDPOINT_URL
        }
    }
    
    # Use get_input_path function from batch.py to get the input file path
    input_file_path = get_input_path(2023, 1)
    print(f"Input file path: {input_file_path}")
    
    # Save the dataframe to S3 using the provided snippet
    df_input.to_parquet(
        input_file_path,
        engine='pyarrow',
        compression=None,
        index=False,
        storage_options=options
    )
    
    print(f"Successfully saved test data to {input_file_path}")
    print(f"DataFrame shape: {df_input.shape}")
    print(f"DataFrame columns: {list(df_input.columns)}")


if __name__ == "__main__":
    test_integration()